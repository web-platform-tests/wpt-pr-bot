...
module.exports = function(number, metadata, dryRun) {
    ...
    if (metadata.reviewedDownstream) {
        ...
        if (dryRun) {
            console.log(`[DRY-RUN] Would approve issue ${number} with message: ${message}`);
    ...
    return promise().then(function() {
        if (metadata.missingAssignee) {
            if (dryRun) {
                console.log(`[DRY-RUN] Would add assignee ${metadata.missingAssignee} to issue ${number}`);
                return Promise.resolve(true);
            }
        ...
        if (metadata.reviewersExcludingAuthor.length > 0 || metadata.isRoot) {
            if (metadata.missingReviewers.length > 0) {
                if (dryRun) {
                    console.log(`[DRY-RUN] Would add reviewers ${metadata.missingReviewers} to issue ${number}`);
