...
function _wgsFromSpecref(spec) {
	...
	var wgs = [];
	if (specref && specref[spec] && specref[spec]["deliveredBy"]) {
...
function getLabelsFromIssue(issue) {
    return github.get("/repos/:owner/:repo/pulls/:number/files", { number: issue }).then(getLabelsFromFiles).then(getLabelsFromSpecs).then(function (list) { return list.map(function (p) { return p.value;}).reduce(function (a,b) { return a.concat(b);})});
}
...
function getLabelsFromSpecs(labels) {    
	...
	if (label == "infra" || label == "assets" || label == "old-tests") {
	    ...
	    deferred = qget.qget("http://specref.jit.su/bibrefs?refs=" + label).then(_wgsFromSpecref(label));
	}
