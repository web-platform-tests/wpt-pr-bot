...
module.exports = async function chooseAssignee(number, metadata) {
  ...
  if (issue.assignee) {
    logger.debug(`#${number}: Already has assignee, will not set missing assignee.`);
    return null;
  }
  ...
  if (reviewers.length) {
    logger.info(`#${number}: Randomly choosing assignee from reviewersExcludingAuthor: ${reviewers}`);
