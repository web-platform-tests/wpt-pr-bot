...
module.exports = function getMetadada(number, author, title, content) {
    ...
    return filenames(number)
        .then(function(filenames) {
            ...
            // reviewers; the latter is used to avoid getting stuck when adding
            // new reviewers to the PR.
            const requestedReviewers = await github.get('/repos/:owner/:repo/pulls/:number/requested_reviewers', { number: number });
            let existingReviewers = requestedReviewers.users.map(function(r) { return r.login.toLowerCase(); });
