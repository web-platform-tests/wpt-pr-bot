...
// labels to 100.
var MAX_LABELS = 100;
var LABEL_FROM_SUBDIRECTORY = new minimatch("@(css|tools)/**");
...
var LABEL_TO_PATTERNS = {
    ...
    "webdriver-bidi": ["webdriver/tests/bidi/**"],
...
var LABEL_TO_MINIMATCH = Object.create(null);
var toPathMinimatch = function(pattern) { return new minimatch(pattern); };

for (var key in LABEL_TO_PATTERNS) {
    LABEL_TO_MINIMATCH[key] = LABEL_TO_PATTERNS[key]
        .map(toPathMinimatch);
}

function fromFile(filename) {
    var matches = function(minimatch_obj) { return minimatch_obj.match(filename); };
    var labels = [];
    var label;

    for (label in LABEL_TO_MINIMATCH) {
        if (LABEL_TO_MINIMATCH[label].some(matches)) {
            labels.push(label);
        }
    }

    if (LABEL_FROM_SUBDIRECTORY.match(filename)) {
