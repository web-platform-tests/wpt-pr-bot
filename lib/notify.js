...
exports.prefixLabel = prefixLabel;
function prefixLabel(label) {
	return label + "-tests";
}

exports.notifyPullRequest = notifyPullRequest;
function notifyPullRequest(data, metadata) {
    return github.get(data.pull_request.user.url).then(function(user) {
        var context = {
            ...
            number: data.number,
            labels: Object.keys(metadata.specs).map(prefixLabel).sort(),
            email: SENDER_EMAIL,
            timestamp: Date.now()
        };
        var body;
...
function notifyComment(data, metadata) {
